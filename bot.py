...
if not TOKEN:
    logger.warning("Nenhum token Telegram encontrado nas variáveis de ambiente (TELEGRAM_TOKEN/BOT_TOKEN)")
URL = f"https://api.telegram.org/bot{TOKEN}/" if TOKEN else ""


def get_db_connection():
    """Cria e retorna uma conexão com o banco de dados Postgres via pg8000.

    Espera as variáveis de ambiente: PGHOST, PGUSER, PGPASSWORD, PGDATABASE, PGPORT (opcional).
    ...
    try:
        ...
        database = os.environ.get("PGDATABASE", os.environ.get("DB_NAME"))
        ...
        logger.error(f"Erro ao criar conexão com o banco: {e}")
        return None


def init_db(retries: int = 3):
    """Garante que a tabela `player_tetas` exista. Retorna True se ok."""
    attempt = 0
    while attempt < retries:
        ...
        try:
            ...
            if conn is None:
                raise RuntimeError("Conexão retornou None")
...
def send_message(chat_id, text):
    try:
        if not URL:
            logger.error("URL do Telegram não configurada. Mensagem não enviada.")
            return
        url = URL + "sendMessage"
...
def get_updates(offset=None):
    if not URL:
        logger.error("URL do Telegram não configurada. get_updates abortado.")
